...
{
    /// <summary>
    /// Manages a shared cache for the extracted Android .tar.bz2 archive.
    /// Downloads and extracts once, reuses across multiple architecture installs.
    /// </summary>
    internal static class AndroidArchiveCache
    {
        private const string CacheFolderName = "SherpaOnnx_AndroidCache";
        private const string DownloadFolderName = "SherpaOnnx_AndroidDownload";
        private const string JniLibsFolder = "jniLibs";
        ...
        internal static bool IsReady
        {
            get
            {
                ...
                string[] dirs = Directory.GetDirectories(cachePath, JniLibsFolder, SearchOption.AllDirectories);
        ...
        internal static void Clean()
        {
            ...
            try
            {
                ...
                Debug.Log("[SherpaOnnx] Android cache cleaned.");
                OnCacheChanged?.Invoke();
            }
            catch (Exception ex)
            {
                Debug.LogError($"[SherpaOnnx] Failed to clean Android cache: {ex.Message}");
            }
        }
            ...
            CancellationToken ct)
        {
            if (IsReady)
            {
                OnStatus?.Invoke("Android cache ready, skipping download.");
            ...
            {
                // Download
                OnStatus?.Invoke("Downloading Android archive...");
                ...
                // Extract
                OnStatus?.Invoke("Extracting Android archive...");
                ...
                OnStatus?.Invoke("Android archive cached.");
                OnProgress01?.Invoke(1f);
                OnCacheChanged?.Invoke();
            }
            finally
            {
                // Always clean up the download temp folder
        ...
        /// Searches recursively because the archive may have a root folder.
        /// </summary>
